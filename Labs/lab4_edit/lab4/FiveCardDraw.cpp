...
int FiveCardDraw::before_betting(){
	...
	for (size_t i = 1; i <= num_players_in_game(); ++i){
		cout << (*players[(dealer_position + i) % num_players_in_game()]).player_name << ": do you want to check or bet ? " << endl;
		getline(cin, response);
		...
		if (response.compare("bet") == 0){
	        ...
	        //can only bet 1 or 2
			while (!(stoi(response) == 1 || stoi(response) == 2)){
			...
			most_recent_bet_position = i;


			//update player's bet so far
			(*players[(dealer_position + i) % num_players_in_game()]).set_bet_so_far(current_bet_value);
...
int FiveCardDraw::betting(){
	...
	//one player bets
		...
		current_position = most_recent_bet_position + 1;
		string response;
		...
		while (!is_betting_end()){
			...
			if ((*players[current_position]).status != out_of_round){
...
int FiveCardDraw::player_bet(std::shared_ptr<Player> &player){
	...
	//player runs out of chips, on obligation to call, bet, or raise
