...
int TexasHoldEm::first_turn(Player & player){
	Card temp;
...
int TexasHoldEm::middle_turn(Player & player){
	Card temp;
	for (unsigned int i = 0; i < 3; ++i){
		temp = main_deck.remove();
		temp.isShared = true;
		player.add_card(temp);
		sharedCards.insert(temp);
	}
...
int TexasHoldEm::final_turns(Player & player){
...
//betting phase
int TexasHoldEm::participate_betting(){
...
int TexasHoldEm::before_betting(size_t position){
...
int TexasHoldEm::after_betting(){
	...
	*/
	...
	for (size_t i = 0; i < num_players_in_game(); ++i){
		if (player_rank(players[0], players[i]) == player_rank(players[i], players[0]) && (*players[i]).status != out_of_round){
...
int TexasHoldEm::player_bet(std::shared_ptr<Player> &player){
...
bool TexasHoldEm::is_betting_end(){
...
void TexasHoldEm::reset_betting_status(size_t raise_index){
	for (size_t i = 1; i < num_players_in_game(); ++i){
...
void TexasHoldEm::reset_all_status(){
	for (size_t i = 0; i < num_players_in_game(); ++i){
...
int TexasHoldEm::reset_afte_firstBetting(){
...
void TexasHoldEm::print_betting_stats(){
	cout << "/////////////////////////////////////////////////////////////////////////////////////////////////////////////" << endl;
...
int TexasHoldEm::firstphase(){
	...
	participate_betting();
	cout << "after particpate" << endl;
	before_betting(dealer_position);
	cout << "after betting" << endl;
	betting();
...
int TexasHoldEm::middle_phase(){

	cout << "Begin Second Phase--Calling Second turn on players-----" << endl;

	for (size_t count = 1; count <= num_players_in_game(); count++){
		//call turn on the player
		middle_turn(*players[(dealer_position + count) % num_players_in_game()]);
...
int TexasHoldEm::final_phase(){
	cout << "Begin Final Phase--Calling Second turn on players-----" << endl;
	for (size_t count = 1; count <= num_players_in_game(); count++){
		//call turn on the player
		final_turns(*players[(dealer_position + count) % num_players_in_game()]);
...
int TexasHoldEm::best_five_forall(){
	for (size_t i = 0; i < num_players_in_game(); ++i){
...
void TexasHoldEm::print_sorted_player(){
	cout << "/////////////////////////////////////////////////////////////////////////////////////////////////////////////" << endl;
...
void TexasHoldEm::highest_automated_leave(string automated_name){
...
void TexasHoldEm::lowest_automated_leave(string automated_name){
	unsigned int random_num = rand() % 100;
...
void TexasHoldEm::other_automated_leave(string automated_name){
	unsigned int random_num = rand() % 100;
	if (random_num <= 49){
		for (size_t i = 0; i < num_players_in_game(); i++)
		{
...
void TexasHoldEm::print_players_cards(shared_ptr<Player> player){
	cout << "Your Card: ^_^" << endl;
	cout << (*player).player_name << "'s cards: " << (*player).getPlayerHand().noShareAsString() << endl;
}


void TexasHoldEm::print_original_vs_bestfive(){
	cout << "/////////////////////////////////////////////////////////////////////////////////////////////////////////////" << endl;
...
void TexasHoldEm::print_shared_cards(){
	cout << "************  Shared Cards *********************" << endl;
	for (Card c : sharedCards){
		cout << c.visibleAsString() << " ";
	}
	cout << endl;
}