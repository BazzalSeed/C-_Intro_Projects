...
pre:
post:
1. take in (open) the file
2. navigate through  the file and create the card object correspondingly
3. Store the card to the vector
*/
{
	...
	while (infile.good()){
		...
		while (current >> word){
			...
			int temp = word.size();
			if (temp != 3 && temp != 2){
				cerr << "Warning!!!Push onto deck: invalid string element type 1" << endl;
				break;
			}
			if (temp == 3){
				...
				if (mymap.find(srank) == mymap.end()){
					cerr << "Warning!!! Push onto deck: invalid string element type 2_a" << endl;
					break;
				}
				if (mymap.find(ssuite) == mymap.end()){
					cerr << "Warning!!! Push onto deck: invalid string element type 2_b" << endl;
					break;
				}
